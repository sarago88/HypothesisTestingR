...
## Exercise 2: Do automatic and manual cars (the "am" variable) get significantly different mileage?
...
levels(mtcars$am) <- c("___", "___")
mtcars$vs <- factor(mtcars$vs)
levels(mtcars$vs) <- c("___", "___")
...
car_anova <- ___(___ ~ ___ * ___, data = ___)
anova(car_anova)
summary(car_anova)
...
car_model <- ___(___ ~ ___ + ___, data = ___)
summary(car_model)